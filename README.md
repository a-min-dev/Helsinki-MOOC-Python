...
This respository outlines progress through the University of Helsinki's Python Programming MOOC 2026, January 2026 to April 2026 (Parts 1 - 13 complete)

## Featured Portfolio
*The following selections demonstrate milestones achieved while completing exercises/projects outlined in the Helsinki MOOC Python & Object Oriented Programming courses (currently working on curating/selecting most relevant projects):*
...
* **[shopping_list.py](./Portfolio/shopping_list.py)** - The program includes a class, ShoppingList, with several methods, such as a method to add an item and the the number of units associated with the item to a shopping list.  The program includes a function to determine the total number of units in a shopping list
...
* **[robot_invasion.py](./Portfolio/robot_invasion/robot_invasion.py)** - The program utilizes the pygame module to create an animation of random robots falling from the sky (the top of the screen) to the ground (the bottom of the screen).  Once a robot hits the ground, the robot randomly moves to the left or to the right until the robot moves off the screen's display
* **[analog_clock.py](./Portfolio/analog_clock.py)** - The program utilizes the pygame module to design an analog clock that displays the current system time
* **[asteroid_robot/main.py](./Portfolio/asteroid_robot/main.py)** - The program utilizes the pygame module to design a game where a player controls a robot character to catch falling asteroid objects.  The gameplay difficulty increases when the player reaches a certain threshold.  The game ends once an asteroid reaches the ground.  The original code has been restructured to include a new config.py file to store application settings and parameters and another file, dynamics.py, to replace code in main.py with functions


## Tech & Tools
* **Language:** Python 3.x
* **Environment:** VS Code + TMC(Test My Code) Extension + Jupyter Notebook
