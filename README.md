...
This respository outlines my progress through the University of Helsinki's Python Programming MOOC 2026 from January 2026 to April 2026 (Parts 1 - 13 completed). The journey continues with data science concepts, including NumPy and Pandas
...
* **[analog_clock.py](./Portfolio/analog_clock.py)** - The program utilizes the pygame module to design and display an analog clock with moving second, minute, and hour hands, showing the current system time
* **[asteroid_robot/main.py](./Portfolio/asteroid_robot/main.py)** - The program utilizes the pygame module to design a game where a player controls a robot character to catch falling asteroid objects.  The gameplay difficulty increases when the player reaches a certain threshold.  The game ends once an asteroid reaches the ground.  The original code is in the process of being restructured to include a config.py to store application settings and parameters and dynamics.py to replace code in main.py with functions
