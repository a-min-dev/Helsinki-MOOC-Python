...
* **[asteroid_robot/main.py](./Portfolio/asteroid_robot/main.py)** - The program utilizes the pygame module to design a game where a player controls a robot character to catch falling asteroid objects.  The gameplay difficulty increases when the player reaches a certain threshold.  The game ends once an asteroid reaches the ground.  The original code has been restructured to include a config.py to store application settings and parameters and a dynamics.py to replace code in main.py with functions
